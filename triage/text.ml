...
let format
    ?(attr=Ansi.Style.default)
    ?(fill=Ansi.Style.default)
    ?(width=78)
    ?(just=`none)
    stream
    =
...
let max_width_over_all_lines stream =
  let aux m l = max m (line_width l) in
    LazyList.fold_left aux 0 stream
  ...
  match cell with
	...
	begin match x with
	  ...
	  | RLineBreak -> 
	      fprintf outc "LBR\n"
	  | RAttr c ->
	      fprintf outc "ATTRS(%s) " (Ansi.Style.to_sequence c)
	end;
...
type printer = {
  pr_ansi : bool;
  mutable pr_attr : Ansi.Style.t option;
...
let print_ansi pr attr =
  let seq = 
    match pr.pr_attr with 
      | None -> Ansi.Style.to_sequence Ansi.Style.default
      | Some pr_attr -> Ansi.Style.transition_to_sequence pr_attr attr
  in
...
let flush pr =
  if pr.pr_ansi then output_string stdout Ansi.Sequence.reset;
  pr.pr_attr <- None;
  flush pr.pr_outc;
  ()
